document.addEventListener('DOMContentLoaded', () => {
    ...
    const loginNameInput = document.getElementById('login-summoner-name');
    const loginRegionSelect = document.getElementById('login-region');
    ...
    function renderStatistics(champs) {
    ...
    async function loadAramData() {
        ...
        if (!grid)
            return;
        grid.innerHTML = aramData.map((item) => {
            const bonusesHtml = item.bonuses.map((b) => `
            ...
            `).join('');
            const augmentsHtml = item.augments.map((aug) => `
    ...
    async function detectAccount() {
        if (loginStatus)
            loginStatus.textContent = 'Connecting to Riot Servers...';
        ...
        try {
            ...
            if (res && res.success) {
                ...
                if (loginStatus)
                    loginStatus.textContent = `Found Account: ${currentUser.name}!`;
    ...
    async function attemptAutoLogin(name, region) {
        try {
    ...
    function updateSummonerUI(data) {
        ...
        if (summonerRankField)
            summonerRankField.textContent = data.rank;
        if (summonerIcon) {
        ...
        if (dhAvatar)
            dhAvatar.style.backgroundImage = `url(https://ddragon.leagueoflegends.com/cdn/14.4.1/img/profileicon/${data.iconId}.png)`;
        ...
        if (summary) {
            if (data.allRanks && data.allRanks.length > 0) {
                const rankStrings = data.allRanks.map((r) => {
    ...
    async function fetchMatchHistory(puuid, region) {
        try {
            ...
            if (historyList) {
                ...
                if (matches.length > 0) {
                        ...
                        dhRecentPerf.innerHTML = `
                            <div style="padding: 24px;">
                                ...
                                <div style="display:flex; gap: 6px; height: 16px; border-radius: 8px; overflow: hidden; background: rgba(255,255,255,0.05); margin-bottom: 12px;">
                                    ${matches.map((m) => `<div style="flex:1; background: ${m.remake ? 'var(--text-muted)' : (m.win ? 'var(--success)' : 'var(--danger)')}; opacity: 0.8;"></div>`).join('')}
                                </div>
                    ...
                    historyList.innerHTML = matches.map((match) => {
                        ...
                        // Items
                        const renderItem = (id, isTrinket = false) => {
                            if (!id || id === 0)
                                return `<div class="b-item empty ${isTrinket ? 'trinket' : ''}"></div>`;
                        ...
                        if (match.largestMultiKill >= 3)
                            badgesHtml += `<div class="b-badge gold"><i class="fa-solid fa-medal"></i> Dreifachtötung!</div>`;
                        else if (match.largestMultiKill === 2)
                            badgesHtml += `<div class="b-badge"><i class="fa-solid fa-dragon"></i> Doppeltötung</div>`;
                        if (match.firstBloodKill)
                            badgesHtml += `<div class="b-badge"><i class="fa-solid fa-droplet" style="color:#ef4444;"></i> Erste Tötung</div>`;
                        if (Number(kdaRatio) >= 3.0)
                            badgesHtml += `<div class="b-badge"><i class="fa-solid fa-fist-raised"></i> Gute KDA</div>`;
                        if (match.totalDamageDealtToChampions > 20000)
                            badgesHtml += `<div class="b-badge"><i class="fa-solid fa-fire"></i> Guter Schaden</div>`;
                        return `
    ...
    // Admin View Logic
    const adminDiscordId = document.getElementById('admin-discord-id');
    ...
    const adminAnnounceAuthor = document.getElementById('admin-announce-author');
    ...
    notifBtn?.addEventListener('click', () => {
        if (notifFlyout) {
            ...
            if (!isVisible && notifDot)
                notifDot.style.display = 'none'; // Clear badge when opened
        }
    });
    // --- Core Loops ---
    ...
    async function renderFeaturedGames() {
        ...
        if (header)
            header.textContent = "GLOBAL FEATURED LIVE MATCHES";
        ...
        participantsList.innerHTML = topGames.map((game) => {
            ...
            const previewChamps = game.participants.slice(0, 3).map((p) => {
                const champInfo = allChampions.find((c) => c.key === String(p.championId));
    ...
    async function updateLiveUI(game) {
        ...
        const participantsList = document.querySelector('.participants-list');
        const selfMatch = game.participants.find((self) => self.puuid === currentUser.puuid) || game.participants[0];
        if (participantsList) {
            ...
            participantsList.innerHTML = game.participants.map((p) => {
                ...
                const champInfo = allChampions.find((c) => c.key === champKeyString);
        ...
        if (liveMode)
            liveMode.textContent = game.gameMode;
        if (selfMatch && overlayChampName) {
            ...
            if (icon)
                icon.style.backgroundImage = `url(https://ddragon.leagueoflegends.com/cdn/14.4.1/img/champion/${champImage})`;
        }
        if (overlayTeamList && selfMatch) {
            overlayTeamList.innerHTML = game.participants.filter((p) => p.teamId === selfMatch.teamId).map((ally) => `
