...
export async function fetchMatchHistory(puuid, region) {
    try {
        ...
        historyList.innerHTML = matches.map((m) => _buildMatchCard(m, puuid)).join('');
    }
    catch (err) {
...
function _renderDashboardPerformance(matches) {
...
function _buildMatchCard(match, puuid) {
    // Ergebnis
    ...
    // Items
    const item = (id, isTrinket = false) => !id || id === 0
    ...
    if (match.largestMultiKill >= 3)
        badges += `<div class="b-badge gold"><i class="fa-solid fa-medal"></i> Dreifachtötung!</div>`;
    else if (match.largestMultiKill === 2)
        badges += `<div class="b-badge"><i class="fa-solid fa-dragon"></i> Doppeltötung</div>`;
    if (match.firstBloodKill)
        badges += `<div class="b-badge"><i class="fa-solid fa-droplet" style="color:#ef4444;"></i> Erste Tötung</div>`;
    if (Number(kda) >= 3.0)
        badges += `<div class="b-badge"><i class="fa-solid fa-fist-raised"></i> Gute KDA</div>`;
    if (match.totalDamageDealtToChampions > 20000)
        badges += `<div class="b-badge"><i class="fa-solid fa-fire"></i> Guter Schaden</div>`;
    return `
